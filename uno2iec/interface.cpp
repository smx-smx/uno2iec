...
// Version      : 0.1
// Target MCU   : Arduino Uno AtMega328(H, 5V) at 16 MHz, 2KB SRAM, 32KB flash,
// 1KB EEPROM.
//
// CREDITS:
// --------
...
//
// DESCRIPTION:
// This "interface" class is the main driving logic for the IEC command
// handling.
//
...
Interface::Interface(IEC &iec)
    : m_iec(iec)
      ...
      m_pDisplay(0)
#endif
      // NOTE: Householding with RAM bytes: We use the middle of serial buffer
      // for the ATNCmd buffer info.
      // This is ok and won't be overwritten by actual serial data from the
      // host, this is because when this ATNCmd data is in use
      // only a few bytes of the actual serial data will be used in the buffer.
      ,
      m_cmd(*reinterpret_cast<IEC::ATNCmd *>(
          &serCmdIOBuf[sizeof(serCmdIOBuf) / 2])) {
...
void Interface::sendStatus(void) {
  ...
  // Length does not include the CR, write all but the last one should be with
  // EOI.
...
void Interface::sendLine(byte len, char *text, word &basicPtr) {
  ...
  // note: minus two here because the line number is included in the array
  // already.
  basicPtr += len + 5 - 2;
...
void Interface::sendListing() {
  ...
  interrupts();
  // This will be slightly tricker: Need to specify the line sending protocol
  // between Host and Arduino.
  ...
  do {
    ...
    resp = serCmdIOBuf[0];
    if ('L' == resp) { // Host system will give us something else if we're at
                       // last line to send.
      // get the length as one byte: This is kind of specific: For listings we
      // allow 256 bytes length. Period.
      byte len = serCmdIOBuf[1];
      // WARNING: Here we might need to read out the data in portions. The
      // serCmdIOBuf might just be too small
      // for very long lines.
      ...
      if (len == actual) {
        ...
        sprintf_P(serCmdIOBuf, (PGM_P)F("Expected: %d chars, got %d."), len,
                  actual);
  ...
  } while ('L' == resp); // keep looping for more lines as long as we got an 'L'
                         // indicating we haven't reached end.
...
void Interface::sendFile() {
  ...
  word bytesDone = 0, totalSize = (((word)((byte)serCmdIOBuf[1])) << 8) bitor
                                  (byte)(serCmdIOBuf[2]);
#ifdef USE_LED_DISPLAY
  ...
  bool success = true;
  // Initial request for a bunch of bytes, here we specify the read size for
  // every subsequent 'R' command.
  // This begins the transfer "game".
  COMPORT.write('N');                   // ask for a byte/bunch of bytes
  COMPORT.write(MAX_BYTES_PER_REQUEST); // specify the arduino serial library
                                        // buffer limit for best performance /
                                        // throughput.
  do {
    ...
    if ('B' == resp or 'E' == resp) {
      ...
      if ('E' not_eq resp)  // if not received the final buffer, initiate a new
                            // buffer request while we're feeding the CBM.
        COMPORT.write('R'); // ask for a byte/bunch of bytes
#endif
      // so we get some bytes, send them to CBM.
      for (byte i = 0; success and i < len;
           ++i) { // End if sending to CBM fails.
  ...
  } while (resp == 'B' and success); // keep asking for more as long as we don't
                                     // get the 'E' or something else
                                     // (indicating out of sync).
  // If something failed and we have serial bytes in recieve queue we need to
  // flush it out.
...
#endif
  if (success) {
    sprintf_P(serCmdIOBuf, (PGM_P)F("Transferred %u of %u bytes."), bytesDone,
              totalSize);
...
void Interface::saveFile() {
  ...
  do {
    ...
    do {
      ...
      interrupts();
      done = (m_iec.state() bitand IEC::eoiFlag) or
             (m_iec.state() bitand IEC::errorFlag);
    } while ((bytesInBuffer < 0xf0) and not done);
    // indicate to media host that we want to write a buffer. Give the total
    // length including the heading 'W'+length bytes.
  ...
  // Did anything happen from the host side?
...
#ifdef CONSOLE_DEBUG
    {
      sprintf_P(serCmdIOBuf,
                (PGM_P)F("ATN code:%d cmd: %s (len: %d) retATN: %d"),
                m_cmd.code, m_cmd.str, m_cmd.strLen, retATN);
      Log(Information, FAC_IFACE, serCmdIOBuf);
    }
#endif
    ...
    case IEC::ATN_CODE_OPEN:
      // Open either file or prg for reading, writing or single line command on
      // the command channel.
      // In any case we just issue an 'OPEN' to the host and let it process.
      // Note: Some of the host response handling is done LATER, since we will
      // get a TALK or LISTEN after this.
      // Also, simply issuing the request to the host and not waiting for any
      // response here makes us more
      // responsive to the CBM here, when the DATA with TALK or LISTEN comes in
      // the next sequence.
    ...
    case IEC::ATN_CODE_DATA: // data channel opened
      if (retATN == IEC::ATN_CMD_TALK) {
        // when the CMD channel is read (status), we first need to issue the
        // host request. The data channel is opened directly.
        if (CMD_CHANNEL == chan)
          handleATNCmdCodeOpen(m_cmd);  // This is typically an empty command,
        handleATNCmdCodeDataTalk(chan); // ...but we do expect a response from
                                        // PC that we can send back to CBM.
      } else if (retATN == IEC::ATN_CMD_LISTEN)
        handleATNCmdCodeDataListen();
      else if (retATN == IEC::ATN_CMD) // Here we are sending a command to PC
                                       // and executing it, but not sending
                                       // response
        handleATNCmdCodeOpen(m_cmd);   // back to CBM, the result code of the
                                       // command is however buffered on the PC
                                       // side.
      break;
...
void Interface::setDateTime(word year, byte month, byte day, byte hour,
                            byte minute, byte second) {
  // Querying the time after this moment will be limited to almost 50-day
  // runtime (after that it will wrap around),
  // thats a limitation that we accept. If querying once between this day
  // interval another 50 day limit is given
...
void Interface::updateDateTime() {
  static const byte daysMonths[] = {31, 28, 31, 30, 31, 30,
                                    31, 31, 30, 31, 30, 31};
  ...
  // now we have to count up (offset) the time with the elapsed millis since it
  // was set last time.
...
char *Interface::dateTimeString(char *dest, bool timeOnly) {
  ...
  // format the string(s).
  if (not timeOnly)
    sprintf_P(dest, (PGM_P)F("%04u02u02u "), m_year, (word)m_month,
              (word)m_day);
  else
    dest[0] = '\0';
  byte timePos = strlen(dest);
  sprintf_P(&dest[timePos], (PGM_P)F("%02u02u02u"), (word)m_hour,
            (word)m_minute, (word)m_second);
...
void Interface::handleATNCmdCodeOpen(IEC::ATNCmd &cmd) {
  ...
  // NOTE: Host side handles BOTH file open command AND the command channel
  // command (from the cmd.code).
...
void Interface::handleATNCmdCodeDataListen() {
  ...
  if (not lengthOrResult or '>' not_eq serCmdIOBuf[0]) {
    // FIXME: Check what the drive does here when things go wrong. FNF is
    // probably not right.
    m_iec.sendFNF();
    strcpy_P(serCmdIOBuf, (PGM_P)F("response not sync."));
    Log(Error, FAC_IFACE, serCmdIOBuf);
...
void Interface::handleATNCmdClose() {
  // handle close of file. Host system will return the name of the last loaded
  // file to us.
  COMPORT.print("C");
  COMPORT.readBytes(serCmdIOBuf, 2);
  byte resp = serCmdIOBuf[0];
  if ('N' == resp or 'n' == resp) { // N indicates we have a name. Case
                                    // determines whether we loaded or saved
                                    // data.
    ...
    if (len == actual) {
      ...
      strncat((char *)scrollBuffer, (char *)serCmdIOBuf,
              sizeof(scrollBuffer) - strlen((char *)scrollBuffer));
