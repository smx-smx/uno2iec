...
// In host mode, the Arduino receives commands via serial line and translates
// them
// into appropriate IEC bus signals.
//
// In device mode, the Arduino listens for commands on the IEC bus and
// translates
...
enum  {
        ...
        // states 1 -> NumInterfaceStates are also valid, representing what's
open
        IS_D64 = 1,
        IS_T64 = 2,
        IS_M2I = 3,
...
public:
  ...
  // The handler returns the current IEC state, see the iec_driver.hpp for
  // possible states.
  // It is called repeatedly in a loop and will either poll the IEC bus (in
  // device mode) or the serial line (in host mode)
  // for commands.
  byte handler(void);

  // Keeping the system date and time as set on a specific moment. The millis()
  // will then keep the elapsed time since
  // moment the time was set.
  void setDateTime(word year, byte month, byte day, byte hour, byte minute,
                   byte second);
  // retrieve the date and time as strings. Current time will be updated
  // according to the elapsed millis before formatting.
  // String will be of format "yyyymmdd hhmmss", if timeOnly is true only the
  // time part will be returned as
