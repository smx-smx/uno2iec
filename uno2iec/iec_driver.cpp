...
// NOTE: Deprecated: Only startup values used in ctor, this will be defined from
// host side.
...
IEC::IEC(byte deviceNumber)
    : m_state(noFlags), m_deviceNumber(deviceNumber), m_atnPin(DEFAULT_ATN_PIN),
      m_dataPin(DEFAULT_DATA_PIN), m_clockPin(DEFAULT_CLOCK_PIN),
      /*m_srqInPin(DEFAULT_SRQIN_PIN),*/ m_resetPin(DEFAULT_RESET_PIN)
#ifdef DEBUGLINES
      ,
      m_lastMillis(0)
#endif
...
//
// FIXME: m_iec might be better returning bool and returning read byte as
// reference in order to indicate any error.
...
IEC::ATNCheck IEC::checkATN(ATNCmd &cmd) {
  ...
  if (not readATN()) {
    // Attention line is active, go to listener mode and get message. Being fast
    // with the next two lines here is CRITICAL!
    writeDATA(true);
    writeCLOCK(false);
    ...
    if (c == (ATN_CODE_LISTEN bitor m_deviceNumber)) {
      ...
      // If the command is DATA and it is not to expect just a small command on
      // the command channel, then
      // we're into something more heavy. Otherwise read it all out right here
      // until UNLISTEN is received.
      if ((c bitand 0xF0) == ATN_CODE_DATA and
          (c bitand 0xF) not_eq CMD_CHANNEL) {
        // A heapload of data might come now, too big for this context to handle
        // so the caller handles this, we're done here.
        ...
        for (;;) {
          ...
          if (i >= ATN_CMD_MAX_LENGTH) {
            ...
            // FIXME: here we should propagate the error type being overflow so
            // that reading error channel can give right code out.
            return ATN_ERROR;
          }
          cmd.str[i++] = c;
        }
      ...
      // First byte is cmd code, that we CAN at least expect. All else depends
      // on ATN.
      c = (ATNCommand)receiveByte();
      if (m_state bitand errorFlag)
        return ATN_ERROR;
...
//
byte IEC::receive() { return receiveByte(); } // receive
...
boolean IEC::init() {
  ...
  digitalWrite(m_resetPin,
               false); // only early C64's could be reset by a slave going high.
#endif
...
void IEC::testINPUTS() {
  ...
  if (now - m_lastMillis >= 1000) {
    ...
    sprintf(buffer, "Lines, ATN: %s CLOCK: %s DATA: %s",
            (readATN() ? "HIGH" : "LOW"), (readCLOCK() ? "HIGH" : "LOW"),
            (readDATA() ? "HIGH" : "LOW"));
    Log(Information, FAC_IEC, buffer);
  }
} // testINPUTS
...
void IEC::testOUTPUTS() {
  ...
  if (now - m_lastMillis >= 1000) {
    m_lastMillis = now;
    char buffer[80];
    sprintf(buffer, "Lines: CLOCK: %s DATA: %s", (lowOrHigh ? "HIGH" : "LOW"),
            (lowOrHigh ? "HIGH" : "LOW"));
    Log(Information, FAC_IEC, buffer);
...
byte IEC::deviceNumber() const { return m_deviceNumber; } // deviceNumber
