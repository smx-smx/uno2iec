...
static Max7219 *pMax;
// Put ANYTHING cool in here you want to scroll as an initial text on your
// MAX7219!
static const byte PROGMEM myText[] =
    "   WELCOME TO UNO2IEC, 1541 ON ARDUINO BY LARS WADEFALK IN 2015...    ";
// Note: This is the pin configuration for the MAX7219 display, if used (not IEC
// related).
...
// Note: This ONLY works if device is not paired and connected. So be sure the
// host is not connected to the HC06
// when configuring the baud rate. Also important to use the latest configured
// baud rate before running commands below.
// When device is shipped it is set to 9600. When compiled and uploaded this
// code, power off the BT device completely
...
// delay(1000);
// Then the baudrate which is a value between 1 and C. Recommended settings are
// 38400 or 57600.
// Here is the list: 1:1200, 2:2400, 3:4800, 4:9600, 5:19200, 6:38400, 7:57600,
...
void loop() {
#ifdef DEBUGLINES
  // This is just for testing that the IEC lines are working properly
  // (soldering, connections etc.)
  // Use either the first or second code line below, for outputs it will every
  // second toggle the DATA and CLOCK.
...
static void waitForPeer() {
  char tempBuffer[80];
  unsigned deviceNumber, atnPin, clockPin, dataPin, resetPin, srqInPin, hour,
  ...
  // Now read the whole configuration string from host, ends with CR. If we
  // don't get THIS string, we're in a bad state.
  ...
  if (deviceNumber == 0) {
    sprintf_P(
        tempBuffer,
        (PGM_P)F("CONNECTED, READY FOR SENDING IEC DATA WITH CBM AS HOST."));
  } else {
    sprintf_P(tempBuffer,
              (PGM_P)F("CONNECTED, READY FOR IEC DATA WITH CBM AS DEV %u."),
              deviceNumber);
  }
  Log(Success, 'M', tempBuffer);
  COMPORT.flush();
  sprintf_P(tempBuffer,
            (PGM_P)F("IEC pins: ATN:%u CLK:%u DATA:%u RST:%u SRQIN:%u"), atnPin,
            clockPin, dataPin, resetPin, srqInPin);
  Log(Information, 'M', tempBuffer);
  sprintf_P(tempBuffer,
            (PGM_P)F("Arduino time set to: %04u-%02u-%02u.%02u:%02u:%02u"),
            year, month, day, hour, minute, second);
  Log(Information, 'M', tempBuffer);
} // waitForPeer
